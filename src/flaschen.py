...
class Flaschen(object):
  ...
  def send_array(self, pixels, offset):
    ...
    max_packet_size = 8000  # Adjust as needed
    chunk_size = max_packet_size // (width * depth)
    ...
    for y in range(0, height, chunk_size):
      ...
      if len(chunk_bytes) > max_packet_size:
        raise ValueError("Chunk size exceeds max packet size")
